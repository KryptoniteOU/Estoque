...
{
    // Convert estoque:// to estoque:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("estoque://"))
    {
        uri.replace(0, 12, "estoque:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Estoque.lnk";
}
...
{
    return GetAutostartDir() / "estoque.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Estoque-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  estoque-qt [" + tr("command-line options") + "]                     " + "\n";
